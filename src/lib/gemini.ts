...
{
  "title": "55자 내. '{keyword}' 포함. 예: '{keyword} | 아가두들 골든두들분양'",
  "metaDescription": "140~158자. '{keyword}', 골든두들분양, 버니두들분양. 전화번호 금지",
...
> {
  ...
  return {
    keyword,
    title: String(data.title || `${keyword} | 아가두들 골든두들분양`),
    metaDescription: clampDesc(data.metaDescription || SITE.description),
    metaKeywords: String(
      data.metaKeywords ||
        `${keyword}, 골든두들분양, 버니두들분양, 골든두들분양가, 골든두들키우기, 아가두들`
    ),
    ...
    heroTitleLine2: String(data.heroTitleLine2 || "아가두들"),
...
export function assembleSeoPage(
  ...
  return {
    ...
    heroTitleLine2: partial.heroTitleLine2 || "아가두들",
