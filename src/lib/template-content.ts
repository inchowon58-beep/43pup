...
const METAS = [
  "{kw}에서 메인쿤분양을 고르는 안내입니다. 메인쿤특징·메인쿤크기·메인쿤분양가를 집 생활에 대입했습니다. 메인스테이 갤러리에서 얼굴을 먼저 보세요.",
  "메인쿤입양 전 확인할 점입니다. {kw} 메인쿤키우기, 무료분양 광고 주의점, 사진 상담. 단가는 상담에서 맞춥니다.",
  "{kw} 안내 — 메인쿤분양과 키우기를 비교해 보세요. 메인스테이에서 아이 사진을 먼저 보실 수 있습니다.",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|mainstaydmc-v1`);
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대개 ‘우리 집에 큰 고양이가 맞나’부터 묻습니다. ${brand}는 메인쿤분양을 사진과 집 리듬에 대입해 풀어 드립니다.`,
  ...
  const faqs = [
    {
      q: `여기는 어떤 곳인가요?`,
      a: `메인스테이는 메인쿤분양을 집 기준으로 풀어 드리는 곳입니다. ${kw}로 찾으시는 분께는 특징·크기·분양가와 아이들 얼굴을 함께 안내합니다.`,
    },
    {
      q: `메인쿤은 어떤 성격인가요?`,
  ...
  return {
    ...
    metaKeywords: `${kw}, 메인쿤분양, 메인쿤분양가, 메인쿤크기, 메인쿤성격, 메인쿤키우기, 메인스테이${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
