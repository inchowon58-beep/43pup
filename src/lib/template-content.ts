...
const METAS = [
  "{kw}에서 가족과 함께 살 골든두들을 고르는 안내입니다. 성격·털·입양 순서와 분양 중인 아이 사진을 정리했습니다. 카카오톡 상담.",
  "사진을 보다가 ‘이 아이면 좋겠다’는 마음이 오면 됩니다. {kw} 골든두들 분양, 크기·성별 상담. 단가는 상담에서 안내합니다.",
  "{kw} 안내 — 곱슬 털과 온순한 성격의 골든두들을 소개합니다. 두들갤러리에서 분양 사진을 먼저 보세요.",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|doodlian-v1`);
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대부분 ‘우리 집에 올 아이’를 먼저 보고 싶어 합니다. 골든두들은 사람을 잘 따르고, 곱슬 털이 포근해 아이와 함께 지내기 좋은 중형견으로 알려져 있습니다.`,
        `사진을 보다가 눈이 머무는 아이가 있으면, 그 마음을 메모해 두세요. 분양 중인 골든두들 모습은 메인 두들갤러리에서도 이어서 보실 수 있습니다.`,
        `상담에 필요한 정보는 단순합니다. 거주 지역, 희망 크기·성별, 아이와 함께 사는지 여부입니다. 서류가 없어도 ${kw} 안내를 받을 수 있습니다. ${KAKAO_CTA_HINT}`,
        `이 페이지는 ${kw}에서 골든두들을 고르기 전, 성격·털·입양 순서를 정리한 안내입니다. 실제 가능 아이·비용은 시기에 따라 달라지므로 상담에서 맞춰 보시면 됩니다.`,
      ],
    },
    {
      h2: fill(pick(FLOW_H2, seed + 1)),
      paragraphs: [
        `골든두들 입양은 보통 사진 확인 → 상담 → 방문 또는 추가 사진 → 집으로 맞이하기 순입니다. ${kw}${as} 알아보신다면, 이 순서를 따라가면 부담이 줄어듭니다.`,
        ...
        `집으로 올 때는 첫 사료·산책·미용 주기를 함께 안내합니다. ${kw}를 검색하셨다면, 사진부터 보신 뒤 상담을 여는 것을 권합니다.`,
      ],
    },
    {
      h2: fill(pick(TRUST_H2, seed + 2)),
      paragraphs: [
        `${kw} 비용은 크기, 혈통, 시기에 따라 달라집니다. 단가를 페이지에 단정하지 않는 이유는 같은 키워드라도 아이마다 범위가 다르기 때문입니다.`,
        `상담 때 물어보면 좋은 항목은 지금 분양 가능한 아이, 성별·코트 타입, 포함 항목, 방문 가능 여부입니다. 견적 없이 결정을 재촉하지 않습니다.`,
        `골든두들을 고를 때는 사진의 눈빛과 함께, 가족 생활 패턴도 맞춰 보시면 됩니다. ${kw}${as} 검색하셨다면 이 체크리스트를 기준으로 물어보세요.`,
      ],
    },
    {
      ...
      paragraphs: [
        ...
        `사진을 더 보고 싶으시면 메인 두들갤러리의 ‘분양중인 골든두들 사진보기’로 이동해 주세요. 마음이 가는 아이가 있으면 바로 물어보시면 됩니다.`,
      ],
    },
  ];
  ...
  return {
    ...
    metaKeywords: `${kw}, 골든두들, 골든두들분양, 골든두들입양, 골드두들, 두들분양, 골든두들키우기, 중형견분양, 하이포알러지강아지${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
