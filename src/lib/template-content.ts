...
const METAS = [
  "{kw}에서 골든두들분양·버니두들분양을 고르는 안내입니다. 골든두들성격·골든두들크기·분양가를 정리했습니다. 카카오톡 상담.",
  "골든두들입양 전 확인할 점입니다. {kw} 골든두들키우기, 무료분양 광고 주의점, 사진 상담. 단가는 상담에서 안내합니다.",
  "{kw} 안내 — 골든두들분양과 버니두들분양을 비교해 보세요. 아가두들 갤러리에서 아이 사진을 먼저 보세요.",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|agadoodle-v1`);
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: `네. 메인 갤러리에서 분양 중인 골든두들 사진을 보실 수 있습니다. 글 중간에 있는 사진보기 버튼으로도 바로 이동할 수 있습니다.`,
    },
  ];
  ...
  return {
    ...
    metaKeywords: `${kw}, 골든두들분양, 버니두들분양, 골든두들분양가, 골든두들키우기, 골든두들성격, 골든두들입양, 골든두들무료분양, 골든두들크기, 아가두들${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
