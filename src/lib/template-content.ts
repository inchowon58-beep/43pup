...
const METAS = [
  "{kw}에서 버니두들분양·골든두들분양을 고르는 안내입니다. 버니두들성격·버니두들크기·분양가를 정리했습니다. 카카오톡 상담.",
  "버니두들입양 전 확인할 점입니다. {kw} 버니두들키우기, 무료분양 광고 주의점, 사진 상담. 단가는 상담에서 안내합니다.",
  "{kw} 안내 — 버니두들분양과 골든두들분양을 비교해 보세요. 두들코리아 갤러리에서 아이 사진을 먼저 보세요.",
];
...
const HERO_SUB = [
  ...
  "버니두들성격, 버니두들크기, 키우기 포인트를 먼저 확인",
  "버니두들분양가와 포함 항목은 상담에서 맞춰 드립니다",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|doodlekorea-v1`);
  const kw = keyword.trim() || "버니두들분양";
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대부분 버니두들분양과 골든두들분양 중 어떤 아이가 맞는지부터 보고 싶어 합니다. 버니두들성격은 사람을 잘 따르고, 버니두들크기는 미니·미디엄·스탠다드로 나뉩니다.`,
        `사진을 보다가 눈이 머무는 아이가 있으면 메모해 두세요. 분양 중인 모습은 메인 갤러리에서도 이어서 보실 수 있습니다.`,
        `상담에 필요한 정보는 거주 지역, 희망 버니두들크기·모색·성별, 아이와 함께 사는지 여부입니다. 서류가 없어도 ${kw} 안내를 받을 수 있습니다. ${KAKAO_CTA_HINT}`,
        `이 페이지는 ${kw}에서 버니두들입양 전, 성격·키우기·분양가를 정리한 안내입니다. 실제 가능 아이·버니두들분양가는 시기에 따라 달라지므로 상담에서 맞춰 보시면 됩니다.`,
      ],
    },
    {
      h2: fill(pick(FLOW_H2, seed + 1)),
      paragraphs: [
        `버니두들입양은 보통 사진 확인 → 상담 → 방문 또는 추가 사진 → 집으로 맞이하기 순입니다. ${kw}${as} 알아보신다면, 이 순서를 따라가면 부담이 줄어듭니다.`,
        `갤러리에서 아이 모습을 본 뒤, 원하는 색·성별·버니두들크기를 알려 주시면 지금 만날 수 있는 아이를 안내합니다. 급하셔도, 천천히 고르셔도 됩니다.`,
        `방문이 가능하면 일정을 맞추고, 어렵다면 카카오톡으로 사진을 더 받아 보실 수 있습니다. 결정은 보호자님의 것입니다.`,
        `집으로 올 때는 버니두들키우기 기본(사료·산책·미용)을 함께 안내합니다. ${kw}를 검색하셨다면, 사진부터 보신 뒤 상담을 여는 것을 권합니다.`,
      ],
    },
    {
      h2: fill(pick(TRUST_H2, seed + 2)),
      paragraphs: [
        `${kw} 버니두들분양가는 혈통, 세대, 버니두들크기, 시기에 따라 달라집니다. 단가를 페이지에 단정하지 않는 이유는 같은 키워드라도 아이마다 범위가 다르기 때문입니다.`,
        `상담 때 물어보면 좋은 항목은 지금 분양 가능한 아이, 성별·모색, 포함 항목, 방문 가능 여부입니다. 견적 없이 결정을 재촉하지 않습니다.`,
        `버니두들무료분양 광고는 조건이 불명확한 경우가 많습니다. ${kw}${as} 검색하셨다면 건강·케어 범위를 기준으로 물어보세요.`,
      ],
    },
    {
      ...
      paragraphs: [
        ...
        `사진을 더 보고 싶으시면 메인 갤러리의 ‘분양중인 버니두들 사진보기’로 이동해 주세요. 버니두들분양·골든두들분양 중 마음이 가는 아이가 있으면 바로 물어보시면 됩니다.`,
      ],
    },
  ];
  ...
  const faqs = [
    {
      q: `버니두들은 어떤 성격인가요?`,
      a: `사람을 잘 따르고 듬직한 편입니다. ${kw}로 찾으시는 분들은 아이·가족과 함께 지낼 반려견을 원하는 경우가 많습니다. 개체 차이는 상담에서 안내합니다.`,
    },
    {
      q: `털이 많이 빠지나요?`,
    ...
    {
      q: `버니두들 분양 비용은 얼마인가요?`,
      a: `크기·혈통·모색·시기에 따라 달라집니다. ${kw} 이용 전에는 포함 항목과 범위를 상담에서 확인하는 것이 좋습니다. 단가를 단정하지 않습니다.`,
    },
    {
      q: `${kw} 상담은 어떻게 하나요?`,
    ...
    },
    {
      ...
      a: `네. 메인 갤러리에서 분양 중인 버니두들 사진을 보실 수 있습니다. 글 중간에 있는 사진보기 버튼으로도 바로 이동할 수 있습니다.`,
    },
  ];
  ...
  return {
    ...
    metaKeywords: `${kw}, 버니두들분양, 골든두들분양, 버니두들분양가, 버니두들키우기, 버니두들성격, 버니두들입양, 버니두들무료분양, 버니두들크기, 두들코리아${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
