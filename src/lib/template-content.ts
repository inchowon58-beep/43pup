...
const METAS = [
  "{kw}에서 메인쿤분양을 고르는 안내입니다. 메인쿤특징·메인쿤크기·메인쿤분양가를 키우는 자리에 맞춰 정리했습니다. 메인가드너 정원에서 얼굴을 먼저 보세요.",
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|maingardneryou-v1`);
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대개 ‘우리 집에 큰 고양이가 맞나’부터 묻습니다. ${brand}는 메인쿤분양을 사진과 키우는 자리에 맞춰 안내합니다.`,
  ...
  const faqs = [
    {
      q: `여기는 어떤 곳인가요?`,
      a: `메인가드너는 메인쿤분양을 키우는 자리 기준으로 안내하는 곳입니다. ${kw}로 찾으시는 분께는 특징·크기·분양가와 아이들 얼굴을 함께 보여 드립니다.`,
    },
    {
      q: `메인쿤은 어떤 성격인가요?`,
