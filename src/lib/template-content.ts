...
const METAS = [
  "{kw}에서 미니두들분양·골든두들분양을 고르는 안내입니다. 희망 크기·성격·분양가 요인을 정리했습니다. 카카오톡 상담.",
  "첫 두들 입양 전 확인할 점입니다. {kw} 미니두들 키우기, 무료분양 광고 주의점, 사진 상담. 단가는 상담에서 안내합니다.",
  "{kw} 안내 — 미니두들과 골든두들 크기를 비교해 보세요. 퍼피두들 갤러리에서 아이 사진을 먼저 보세요.",
];
...
const HERO_SUB = [
  ...
  "희망 크기, 성격, 키우기 포인트를 먼저 확인",
  "분양가와 포함 항목은 상담에서 맞춰 드립니다",
];

const INTRO_H2 = [
  "{kw}, 미니두들분양 전에",
  "첫 두들 키우기를 생각하며 보는 {kw}",
  "{kw}와 함께 보는 골든두들분양",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|puppydoodle-v1`);
  const kw = keyword.trim() || "미니두들분양";
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대부분 미니두들분양과 골든두들분양 중 어떤 크기가 맞는지부터 보고 싶어 합니다. 미니·미디엄은 실내 생활에 맞추기 쉽고, 성격은 사람을 잘 따르는 편입니다.`,
        `사진을 보다가 눈이 머무는 아이가 있으면 메모해 두세요. 분양 중인 모습은 메인 갤러리에서도 이어서 보실 수 있습니다.`,
        `상담에 필요한 정보는 거주 지역, 희망 크기·성별, 아이와 함께 사는지 여부입니다. 서류가 없어도 ${kw} 안내를 받을 수 있습니다. ${KAKAO_CTA_HINT}`,
        `이 페이지는 ${kw}에서 첫 두들 입양 전, 크기·키우기·분양가를 정리한 안내입니다. 실제 가능 아이·분양가는 시기에 따라 달라지므로 상담에서 맞춰 보시면 됩니다.`,
      ],
    },
    {
      h2: fill(pick(FLOW_H2, seed + 1)),
      paragraphs: [
        `첫 두들 입양은 보통 사진 확인 → 상담 → 방문 또는 추가 사진 → 집으로 맞이하기 순입니다. ${kw}${as} 알아보신다면, 이 순서를 따라가면 부담이 줄어듭니다.`,
        `갤러리에서 아이 모습을 본 뒤, 원하는 색·성별·크기를 알려 주시면 지금 만날 수 있는 아이를 안내합니다. 급하셔도, 천천히 고르셔도 됩니다.`,
        `방문이 가능하면 일정을 맞추고, 어렵다면 카카오톡으로 사진을 더 받아 보실 수 있습니다. 결정은 보호자님의 것입니다.`,
        `집으로 올 때는 키우기 기본(사료·산책·미용)을 함께 안내합니다. ${kw}를 검색하셨다면, 사진부터 보신 뒤 상담을 여는 것을 권합니다.`,
      ],
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `사진을 더 보고 싶으시면 메인 갤러리의 ‘분양중인 미니두들 사진보기’로 이동해 주세요. 미니두들분양·골든두들분양 중 마음이 가는 아이가 있으면 바로 물어보시면 됩니다.`,
      ],
    },
  ];
  ...
  const faqs = [
    {
      q: `미니두들은 어떤 성격인가요?`,
      a: `사람을 잘 따르고 온순한 편입니다. ${kw}로 찾으시는 분들은 아이·가족과 함께 지낼 첫 두들을 원하는 경우가 많습니다. 개체 차이는 상담에서 안내합니다.`,
    },
    {
      q: `털이 많이 빠지나요?`,
    ...
    {
      q: `아파트에서도 키울 수 있나요?`,
      a: `미니·미디엄은 실내 생활이 가능한 경우가 많습니다. 산책과 사람과의 시간이 중요하니, 생활 패턴을 알려 주시면 ${kw} 기준으로 맞는 아이를 안내합니다.`,
    },
    {
      q: `두들 분양 비용은 얼마인가요?`,
    ...
    },
    {
      ...
      a: `네. 메인 갤러리에서 분양 중인 미니두들 사진을 보실 수 있습니다. 글 중간에 있는 사진보기 버튼으로도 바로 이동할 수 있습니다.`,
    },
  ];
  ...
  return {
    ...
    metaKeywords: `${kw}, 미니두들분양, 골든두들분양, 버니두들분양, 골든두들분양가, 골든두들키우기, 골든두들성격, 골든두들입양, 골든두들크기, 퍼피두들${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
    ...
    heroTitleLine1: kw,
    heroTitleLine2: "퍼피두들",
    heroBar: "미니두들분양·골든두들분양 사진을 보고 크기를 정해 보세요.",
    sections,
    faqs,
