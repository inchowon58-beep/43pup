...
const METAS = [
  "{kw}에서 메인쿤분양을 고르는 안내입니다. 메인쿤특징·메인쿤크기·메인쿤분양가를 정리했습니다. 쿤스토리 갤러리에서 사진을 먼저 보세요.",
  "메인쿤입양 전 확인할 점입니다. {kw} 메인쿤키우기, 무료분양 광고 주의점, 사진 상담. 단가는 상담에서 안내합니다.",
  "{kw} 안내 — 메인쿤분양과 키우기를 비교해 보세요. 쿤스토리에서 아이 사진을 먼저 보실 수 있습니다.",
];
...
const HERO_SUB = [
  ...
  "메인쿤성격, 메인쿤크기, 키우기 포인트를 먼저 확인",
  "메인쿤분양가와 포함 항목은 상담에서 맞춰 드립니다",
];
...
export function generateTemplateContent(keyword: string, pageIndex = 1): SeoPage {
  const seed = hash(`${keyword}|${pageIndex}|${SITE.brand}|maincoonag-v1`);
  ...
  const sections = [
    {
      ...
      paragraphs: [
        `${kw}${obj} 찾는 분들은 대개 메인쿤크기와 성격, 분양가부터 알고 싶어 합니다. ${brand}는 메인쿤분양을 사진과 생활 환경에 맞춰 안내합니다.`,
        ...
        `이 페이지는 ${kw}에서 메인쿤을 고르기 전, 특징·크기·분양가·키우기를 정리한 안내입니다. 실제 가능한 아이와 비용은 상담에서 맞춰 보시면 됩니다.`,
      ],
    },
    {
      h2: fill(pick(FLOW_H2, seed + 1)),
      paragraphs: [
        `${kw} 상담은 보통 사진 보기 → 조건 맞추기 → 방문 또는 추가 사진 → 입양 순입니다. 급하게 결정하지 않아도 됩니다.`,
  ...
  const faqs = [
    {
      q: `여기는 어떤 곳인가요?`,
      a: `쿤스토리는 메인쿤분양을 안내하는 곳입니다. ${kw}로 찾으시는 분께는 특징·크기·분양가와 아이들 사진을 함께 안내합니다.`,
    },
    {
      q: `메인쿤은 어떤 성격인가요?`,
  ...
  return {
    ...
    metaKeywords: `${kw}, 메인쿤분양, 메인쿤분양가, 메인쿤크기, 메인쿤성격, 메인쿤키우기, 쿤스토리${
      geoKw ? `, ${geoKw}` : ""
    }`,
    h1,
    heroSubtitle,
