...
export function buildHugdayPage(site: HugdaySite): HugdayPage {
  ...
  const essayPool = isShelter
    ? [
        [
          ...
          `후원·입양 일정은 이 사이트 입점 안내에서 이어집니다. 이 페이지는 ${kw} 전용입니다.`,
        ],
        [
    ...
    : [
        [
          ...
          `분양가는 시기·개체에 따라 달라 이 글에 단가를 박지 않습니다. 상담 연락처는 입점 후에 이 사이트에 표시됩니다.`,
        ],
        [
          ...
          `${kw} 일정은 입점 안내에서 이어집니다. 이 사이트는 ${name} 전용입니다.`,
        ],
        [
  ...
  const flow = [
    ...
    { n: "03", title: "문의", body: `입점이 되면 이 사이트에 연락처가 표시됩니다. 희망 시기만 알려 주셔도 됩니다.` },
  ...
  const metaDescription = isShelter
    ? `${kw} ${BRAND}. ${name}에서 ${noun} 입양 전 건강·성격·첫 주를 안내합니다.`
