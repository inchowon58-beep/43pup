...
/** 인근 지하철역 — yourdogzone 스타일 SEO 키워드 카드 */
export default function NearbyStationsSection({
  cityLabel,
  stations,
  keywordSuffix = "강아지장례식장",
}: Props) {
  if (stations.length === 0 || !cityLabel) return null;

  return (
    <section className="mb-12 border border-[var(--line)] bg-[linear-gradient(135deg,rgba(248,238,230,0.55),#fffcf7)] p-6 shadow-[0_8px_24px_rgba(42,36,31,0.04)] sm:p-8">
          ...
          return (
            <li key={item.label}>
              {item.href ? (
                <Link
                  href={item.href}
                  className="flex items-start gap-2 border border-[var(--line)] bg-[#fffcf7] px-4 py-3.5 transition hover:border-[var(--coral)]"
                >
                  {body}
                </Link>
              ) : (
