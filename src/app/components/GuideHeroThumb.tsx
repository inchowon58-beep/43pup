...
export default function GuideHeroThumb({ page, imageSrc }: Props) {
  ...
  const bar =
    page.heroBar || page.heroSubtitle || "크기·기질·분양가를 스펙처럼 먼저 맞춰 보세요";
  const real = isRealImage(imageSrc);

  return (
    <div className="relative mx-auto aspect-square w-full max-w-[720px] overflow-hidden rounded-[var(--radius)] shadow-[0_18px_44px_rgba(27,40,56,0.22)] ring-1 ring-[#2a7a6e]/35">
      {real ? (
        <Image
          src={imageSrc}
      ...
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(27,40,56,0.12)_0%,rgba(16,24,32,0.62)_100%)]" />
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-end px-7 pb-8 text-left md:px-10 md:pb-10">
        ...
        <h1 className="mt-4 max-w-[16ch] text-[clamp(1.7rem,6vw,2.9rem)] font-bold leading-[1.25] text-white drop-shadow-[0_3px_10px_rgba(0,0,0,0.4)]">
          ...
          <span className="mt-1 block text-[#d4b896]">{line2}</span>
        </h1>
