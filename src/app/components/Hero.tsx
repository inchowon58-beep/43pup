  ...
  return (
    <section id="top" className="relative min-h-[92svh] overflow-hidden">
      <div className="absolute inset-0 hero-media">
        <ImageSlot index={15} fill priority label={`${SITE.name} 메인쿤분양`} />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(30,36,51,0.48)_0%,rgba(30,36,51,0.12)_40%,rgba(18,22,32,0.92)_100%)]" />
      </div>

      <div className="container relative flex min-h-[92svh] flex-col justify-end pb-28 pt-28 md:pb-24">
        <div className="animate-rise max-w-2xl">
          <p className="text-xs font-semibold tracking-[0.22em] text-[#f0e6d8]">
            {SITE.taglineEn}
          </p>
          ...
          <p className="mt-6 max-w-xl text-sm leading-relaxed text-white/85 md:text-base">
            {SITE.tagline} 갤러리에서 아이 얼굴을 먼저 보신 뒤,{" "}
            {kakaoHref ? "카카오톡으로 이어 주세요." : "조건이 맞으면 상담으로 이어 주세요."}
          </p>
          <p className="mt-4 max-w-xl text-xs leading-relaxed text-[#f6e6dc] md:text-sm">
            {KEYWORD_INQUIRY}
          </p>
