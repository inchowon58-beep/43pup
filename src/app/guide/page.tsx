...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: "지역별 장례 안내",
    description: `${SITE.name} 지역별 강아지 장례 — 지금 아이가 떠났을 때 바로 할 일, 24시 픽업·화장·추모 안내`,
    keywords: [...SITE.keywords, "지역별 강아지장례식장"],
    alternates: { canonical: url },
    openGraph: {
      title: `지역별 장례 안내 | ${SITE.name}`,
      description: `${SITE.name} 24시 장례·화장·추모 지역 가이드`,
      url,
  ...
  return (
    <div className="container min-h-screen py-28">
      ...
      <p className="mt-3 max-w-xl text-[var(--muted)]">
        최신 {Math.min(total, PUBLIC_PAGE_LIMIT)}건 · 24시 장례·화장·추모 지역 가이드
      </p>
      <p className="mt-2 text-sm text-[var(--muted)]">
        오래된 문서는 검색 유입과 직접 URL 접근을 위해 유지되며, 목록에는 최신 문서 중심으로 노출됩니다.
      </p>

      <ul className="mt-10 divide-y divide-[var(--line)] overflow-hidden border border-[var(--line)] bg-[#fffcf7]">
        {slice.length === 0 && (
        ...
        <nav className="mt-8 flex flex-wrap items-center justify-center gap-2" aria-label="페이지">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
            <Link
              ...
              className={`min-w-9 rounded-[0.35rem] px-2 py-1 text-center text-sm ${
                n === current
                  ? "bg-[var(--sky-deep)] text-white"
                  : "border border-[var(--line)] bg-[#fffcf7] text-[var(--ink)]"
              }`}
            >
              {n}
            </Link>
