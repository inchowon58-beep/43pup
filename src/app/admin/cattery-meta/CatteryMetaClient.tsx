...
export default function CatteryMetaClient() {
  ...
  return (
    <div className="admin-page-root container min-h-screen py-28 pb-16">
      ...
      <h1 className="text-3xl font-extrabold text-[var(--navy)]">포옹데이 네이버 메타</h1>
      <p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">
        서브도메인은 견종·묘종·보호소마다 <code>폴더명pet.puppytimes.co.kr</code> 입니다.
        서치어드바이저에서 받은 <code>naver-site-verification</code> 값만 붙여 넣으면 됩니다.
        전체 메타 태그를 붙여도 content 값만 저장합니다. 한 번에 저장하면 43개 사이트 헤드에 반영됩니다.
      </p>
      ...
      <form onSubmit={onSave} className="mt-6">
        <div className="mb-4 flex flex-wrap items-end gap-3">
          <label className="min-w-[16rem] flex-1">
            검색 (품종·서브도메인)
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="말라뮤트, malamutepet" />
          </label>
        ...
        <div className="overflow-x-auto rounded-[var(--radius)] border border-[var(--line)] bg-white">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="bg-[var(--bg)] text-[var(--muted)]">
              <tr>
                <th className="px-3 py-2 font-semibold">품종</th>
