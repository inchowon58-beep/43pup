...
const ALLOWED_REMOTE = /github\.com[:/]+inchowon58-beep\/maincoonmar(\.git)?$/i;
...
if (
  vercel?.projectName &&
  /^(mainyou|maincoonpshop|maincooninfo|maincoondmc|maincoonag|smpinfo|infowedding|pruwedding|dmcwedding|globalwedding|pupmaincoon|enmaincoon|puppydoodle|eanimal|muziga|muzi01|muzi02|catterydoodle|doodle|doodlekorea|agadoodle)$/i.test(
    vercel.projectName
  )
) {
  fail(
    `Vercel 연결이 기존 '${vercel.projectName}' 입니다. 이전 프로젝트는 사용하지 않습니다.\n` +
      `maincoon.marketstore.co.kr / inchowon58-beep/maincoonmar 전용으로 다시 연결하세요.`
  );
}
...
try {
  ...
  fail("git origin이 없습니다. https://github.com/inchowon58-beep/maincoonmar.git 로 설정하세요.");
}
...
if (!ALLOWED_REMOTE.test(remote)) {
  fail(
    `git origin은 https://github.com/inchowon58-beep/maincoonmar.git 이어야 합니다.\n  현재: ${remote}`
  );
}
