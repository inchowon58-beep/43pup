...
def _rng(keyword: str, idx: int) -> random.Random:
    seed = int(hashlib.md5(f"{keyword}|{idx}|dmcwedding".encode()).hexdigest()[:8], 16)
    ...
    sections = [
        {
            ...
            "paragraphs": [
                ...
                f"글로벌 메이트는 특정 업체를 전면에 노출하지 않고, {tone} 확인할 항목을 안내합니다.",
